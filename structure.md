...
│   ├── User.cpp                 # Triển khai class User — constructor, getter/setter, toString() (Đức)
│   ├── Graph.cpp                # Triển khai class Graph — thêm/xóa user & edge, truy vấn neighbors, hiển thị mạng lưới (Đức)
│   ├── CSVHandler.cpp           # Triển khai đọc/ghi CSV — parse file, instant sync (cập nhật file ngay mỗi thao tác)
│   ├── Algorithms.cpp           # Triển khai thuật toán — BFS (queue + visited), mutual friends, friend suggestions, shortest path (Đức)
...
│   └── DSA_report (latex)         # Báo cáo học thuật môn DSA — lý thuyết, thiết kế, phân tích độ phức tạp, kết quả
